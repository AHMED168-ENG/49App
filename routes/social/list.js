  ...
  try {
    ...
    const userIds = await user_model
      .findById(req.user.id, {
        friends: { $slice: [((page ?? 1) - 1) * 20, 20] },
      })
      .select("friends");

    if (userIds.friends.length > 0)
      result = await user_model
        .find({ _id: { $in: userIds.friends } })
        .select(baseUserKeys);
...
router.get("/my-friend-requests", verifyToken, async (req, res, next) => {
  try {
    ...
    const userIds = await user_model
      .findById(req.user.id, {
        friend_requests: { $slice: [((page ?? 1) - 1) * 20, 20] },
      })
      .select("friend_requests");

    if (userIds.friend_requests.length > 0)
      result = await user_model
        .find({ _id: { $in: userIds.friend_requests } })
        .select(baseUserKeys);
...
router.get("/my-blocked", verifyToken, async (req, res, next) => {
  try {
    ...
    const userIds = await user_model
      .findById(req.user.id, {
        block: { $slice: [((page ?? 1) - 1) * 20, 20] },
      })
      .select("block");

    if (userIds.block.length > 0)
      result = await user_model
        .find({ _id: { $in: userIds.block } })
        .select(baseUserKeys);
...
router.get("/my-followers", verifyToken, async (req, res, next) => {
  try {
    ...
    const userIds = await user_model
      .findById(req.user.id, {
        followers: { $slice: [((page ?? 1) - 1) * 20, 20] },
      })
      .select("followers");

    if (userIds.followers.length > 0)
      result = await user_model
        .find({ _id: { $in: userIds.followers } })
        .select(baseUserKeys);
...
router.get("/peer-friends/:userId", verifyToken, async (req, res, next) => {
  try {
    ...
    if (
      (req.user.id != req.params.userId && friendsPrivacy == 1) ||
      (req.user.id != req.params.userId && (friendsPrivacy == 3) & !isFriend) ||
      (req.user.id != req.params.userId &&
        (friendsPrivacy == 4) & !isFollower) ||
      (req.user.id != req.params.userId &&
        (friendsPrivacy == 5) &
          ((req.user.id != req.params.userId && !isFriend) || !isFollower))
    )
      return next({
        status: 400,
        message:
          language == "ar"
            ? "لا تملك صلاحية لروئية هذه الصفحة"
            : "You do not have permission to view this page",
      });

    if (peerData.friends.length == 0)
      return res.json({
        status: true,
        data: [],
      });
    ...
    const users = await user_model.aggregate([
      {
        $match: {
          _id: { $in: peerData.friends.map((e) => mongoose.Types.ObjectId(e)) },
        },
      },
      {
        $project: {
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const searchResult = await user_model
        .find({
...
router.get("/peer-followers/:userId", verifyToken, async (req, res, next) => {
  try {
    ...
    if (
      (req.user.id != req.params.userId && followersPrivacy == 1) ||
      (req.user.id != req.params.userId &&
        (followersPrivacy == 3) & !isFriend) ||
      (req.user.id != req.params.userId &&
        (followersPrivacy == 4) & !isFollower) ||
      (req.user.id != req.params.userId &&
        (followersPrivacy == 5) &
          ((req.user.id != req.params.userId && !isFriend) || !isFollower))
    )
    ...
    if (peerData.followers.length == 0)
      return res.json({
        status: true,
        data: [],
      });
    ...
    const users = await user_model.aggregate([
      {
        $match: {
          _id: {
            $in: peerData.followers.map((e) => mongoose.Types.ObjectId(e)),
          },
        },
      },
      {
...
router.get("/my-profile-viewers", verifyToken, async (req, res, next) => {
  try {
    ...
    const isSubscription =
      (await subscription_model.findOne({
        user_id: req.user.id,
        sub_category_id: profileViewCategoryId,
      })) != null;

    if (!isSubscription)
      return next({ status: 400, message: "not_subscription" });

    const viewers = await profile_view_model
      .find({ owner_id: req.user.id })
      .sort({ createdAt: -1, _id: 1 })
      .skip(((page ?? 1) - 1) * 20)
      .limit(20)
      .select("user_id times updatedAt");
    ...
    const users = await user_model
      .find({ _id: { $in: viewers.map((e) => e.user_id) } })
      .select("first_name last_name profile_picture cover_picture");
