...
router.get("/get-peer/:id", verifyToken, async (req, res, next) => {
  try {
    ...
    if (
      ...
      return next({
        status: 404,
        message: language == "ar" ? "الحساب غير موجود" : "User is Not Exist",
      });
    ...
    var data = {
      ...
      friend_request_enable:
        user.privacy_friend_request == 2 ||
        (isFriend && user.privacy_friend_request == 3) ||
        (isFollower && user.privacy_friend_request == 4) ||
        ((isFollower || isFriend) && user.privacy_friend_request == 5),
      is_message_enable:
        user.privacy_receive_messages == 2 ||
        (isFriend && user.privacy_receive_messages == 3) ||
        (isFollower && user.privacy_receive_messages == 4) ||
        ((isFollower || isFriend) && user.privacy_receive_messages == 5),
      is_call_enable:
        user.privacy_call == 2 ||
        (isFriend && user.privacy_call == 3) ||
        (isFollower && user.privacy_call == 4) ||
        ((isFollower || isFriend) && user.privacy_call == 5),
      is_follow_list_enable:
        user.privacy_follower_list == 2 ||
        (isFriend && user.privacy_follower_list == 3) ||
        (isFollower && user.privacy_follower_list == 4) ||
        ((isFollower || isFriend) && user.privacy_follower_list == 5),
      is_friend_list_enable:
        user.privacy_friend_list == 2 ||
        (isFriend && user.privacy_friend_list == 3) ||
        (isFollower && user.privacy_friend_list == 4) ||
        ((isFollower || isFriend) && user.privacy_friend_list == 5),
    };

    if (
      user.privacy_phone == 2 ||
      (isFriend && user.privacy_phone == 3) ||
      (isFollower && user.privacy_phone == 4) ||
      ((isFollower || isFriend) && user.privacy_phone == 5)
    )
      data.phone = user.phone;

    if (
      user.privacy_is_male == 2 ||
      (isFriend && user.privacy_is_male == 3) ||
      (isFollower && user.privacy_is_male == 4) ||
      ((isFollower || isFriend) && user.privacy_is_male == 5)
    )
      data.is_male = user.is_male;

    if (
      user.privacy_social_status == 2 ||
      (isFriend && user.privacy_social_status == 3) ||
      (isFollower && user.privacy_social_status == 4) ||
      ((isFollower || isFriend) && user.privacy_social_status == 5)
    )
      data.social_status = user.social_status;

    if (
      user.privacy_birth_date == 2 ||
      (isFriend && user.privacy_birth_date == 3) ||
      (isFollower && user.privacy_birth_date == 4) ||
      ((isFollower || isFriend) && user.privacy_birth_date == 5)
    )
      data.birth_date = user.birth_date;

    if (
      user.privacy_job == 2 ||
      (isFriend && user.privacy_job == 3) ||
      (isFollower && user.privacy_job == 4) ||
      ((isFollower || isFriend) && user.privacy_job == 5)
    )
      data.job = user.job;

    if (
      user.privacy_city == 2 ||
      (isFriend && user.privacy_city == 3) ||
      (isFollower && user.privacy_city == 4) ||
      ((isFollower || isFriend) && user.privacy_city == 5)
    )
      data.city = user.city;

    if (
      user.privacy_country == 2 ||
      (isFriend && user.privacy_country == 3) ||
      (isFollower && user.privacy_country == 4) ||
      ((isFollower || isFriend) && user.privacy_country == 5)
    )
      data.country = user.country;

    if (user.referral_id) {
      const referralUser = await user_model
        .findById(user.referral_id)
...
router.post("/send-friend-request", verifyToken, async (req, res, next) => {
  try {
    ...
    const exist = await user_model
      .findOne({ _id: peer_id, friend_requests: { $all: [req.user.id] } })
      .select("_id");
    ...
    if (!user)
      return next({
        status: 400,
        message:
          language == "ar" ? "الحساب ليس موجود" : "The Account is not exist",
      });

    const isAlreadySentRequest = await user_model
      .findOne({ _id: req.user.id, friend_requests: { $all: [peer_id] } })
      .select("_id");
    ...
    if (isAlreadySentRequest) {
      await user_model.updateOne(
        { _id: req.user.id },
        {
          $addToSet: { friends: peer_id, followers: peer_id },
      ...
      await user_model.updateOne(
        { _id: peer_id },
        {
          $addToSet: { friends: req.user.id, followers: req.user.id },
      ...
      return res.json({
        status: true,
        data:
          language == "ar" ? "لقد اصبحتم اصدقاء الان" : "You are now friends",
      });
    } else {
      await user_model
        .updateOne(
      ...
      if (
        (await notification_model.findOne({
          receiver_id: peer_id,
          user_id: req.user.id,
          type: 2,
        })) == null
  ...
  try {
    ...
    if (!exist)
      return next({
        ...
        message:
          language == "ar"
            ? "هذا المستخدم لم يقم بارسال طلب صداقة لك"
  ...
  try {
    ...
    if (!exist)
      return next({
        ...
        message:
          language == "ar"
            ? "لم تقم بارسال طلب صداقة لهذا المستخدم"
            : "You have not sent a friend request to this user",
      });

    await user_model
      .updateOne({ _id: peer_id }, { $pull: { friend_requests: req.user.id } })
      .exec();
...
router.delete("/remove-friend-request", verifyToken, async (req, res, next) => {
  try {
    ...
    await user_model
      .updateOne({ _id: req.user.id }, { $pull: { friend_requests: peer_id } })
      .exec();
...
router.delete("/un-friend", verifyToken, async (req, res, next) => {
  try {
    ...
    const user = await user_model
      ...
      .select("first_name last_name profile_picture cover_picture");
    ...
    const exist = await user_model
      .findOne({ _id: peer_id, friends: { $all: [req.user.id] } })
      .select("_id");

    if (!exist)
      return next({
        status: 400,
        message: language == "ar" ? "انتم لستم اصدقاء" : "You are not friends",
      });

    await user_model
      .updateOne({ _id: peer_id }, { $pull: { friends: req.user.id } })
      .exec();

    await user_model
      .updateOne({ _id: req.user.id }, { $pull: { friends: peer_id } })
      .exec();
...
router.post("/follow", verifyToken, async (req, res, next) => {
  try {
    ...
    const user = await user_model
      .findOne({ _id: peer_id, block: { $nin: [req.user.id] } })
      .select("_id");
    ...
    const exist = await user_model
      .findOne({ _id: peer_id, followers: { $all: [req.user.id] } })
      .select("_id");
  ...
  try {
    ...
    await user_model
      .updateOne(
        { _id: req.user.id },
        {
          $addToSet: { block: peer_id },
          $pull: {
            friends: peer_id,
            followers: peer_id,
            following: peer_id,
            friend_requests: peer_id,
          },
        }
      )
      .exec();
    ...
    post_model
      .updateOne({ user_id: peer_id }, { $pull: { tags: req.user.id } })
      .exec();
    post_model
      .updateOne({ user_id: req.user.id }, { $pull: { tags: peer_id } })
      .exec();

    post_comment_model
      .deleteMany({ post_owner_id: peer_id, user_id: req.user.id })
      .exec();
    post_comment_model
      .deleteMany({ post_owner_id: req.user.id, user_id: peer_id })
      .exec();
...
router.delete("/un-block", verifyToken, async (req, res, next) => {
  try {
    ...
    await user_model
      .updateOne({ _id: req.user.id }, { $pull: { block: peer_id } })
      .exec();
